...
esp_err_t hexdump_to_file(const char *path, char *data, size_t data_bytes) {
    ...
    // ...
    char hex_buffer[4]; // Enough for "XX " and null terminator
    for (size_t i = 0; i < data_bytes; ++i) {
        snprintf(hex_buffer, sizeof(hex_buffer), "%02X ", data[i]);
        fprintf(f, hex_buffer);
    ...
    ESP_LOGI(TAG, "Hexdump file written");
